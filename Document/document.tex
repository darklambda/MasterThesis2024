...
With these in mind, ``jose.backends.native'' and ``slugify.\_\_main\_\_'' are for each case the best modules respectively, with an improvement of \(0.045\) points in branch coverage, and a reduction of \(-99.587\%\) in the amount of iterations, while executing the extension with MIO.\@
...
% TODO - add examples of generated test templates

Altough, considering that the nature of MIO's heuristic is different compared to the other algorithms, and it would have been also relevant to dig further other configurations, there was not any module that met the relevant scenarios previously proposed in any of the alternative algorithms.
...
For these cases, the obtained modules were ``pymonet.utils'', with an increase of \(0.025\) in coverage, and ``py\_backwards.messages'', with a decrease of \(50.10\%\) in the number of computed iterations.
The two modules have an OLR value of \(0.75\) and \(1.0\) respectively.

Listing~\ref{lst:9} shows two out of the three present callables within the ``pymonet.utils'' module that influenced the given OLR value.
In them, the class Callable is an alias of the built-in class CallableType from Python 3.12, that does not hold any attributes in its definition nor need them for reaching the highlighted branches. 

\begin{figure}
\begin{minted}[highlightlines={4, 10}]{python}
@curry
...
def memoize(fn: Callable, key=eq) -> Callable:
    ...
    return memoized_fn
\end{minted}
\caption{Example method from the module pymonet.utils, with highlighted branches.}\label{lst:9}
\end{figure}

% TODO - add examples of generated test templates

For the next case, Listing~\ref{lst:10} presents two callables from the second relevant module with parameters of type \classname{CompilationError} and \classname{TransformationError}, subclasses of the \classname{Exception} class.
The branches of the first function might be subjects of a possible test template, generated including a \classname{CompilationError} object with the attribute ``lineno'' assigned to a certain variable.
Nonetheless, the type annotations of the class are given only in the \callable[]{\_\_init\_\_} method signature, fact that limits the knowledge of the heuristic prior to the OCG to statement translation process.

\begin{figure}
\begin{minted}[highlightlines={6, 8, 13}]{python}
...
def transformation_error(e: TransformationError) -> str:
    return ('{red}{bright}Transformation error in "{e.filename}", '
        ...
        bright=Style.BRIGHT)
\end{minted}
\caption{Example method from the module py\_backwards.messages, with highlighted branches.}\label{lst:10}
\end{figure}
...
An additional example of this behaviour (for an algorithm different to MIO) is the module ``py\_backwards.compiler'', which obtained one of the best \(\hat{A}_{12}\) effect size values for DynaMOSA, but still did not find any coverage improvement.
