...
The values for percentage difference between both approaches over the average value of plotted distributions, the different algorithms get \(-7.20\%\), \(-3.65\%\), \(-6.48\%\) and \(0.92\%\), in the same order from left to right as they are presented in Figure~\ref{fig:iterations}.
...
The data reported in Table~\ref{tab:performance} show the mean coverage of all tested modules per approach, the p-Values obtained through statistical Mann-Whitney U tests and Cohen's D effect size measures, in any pair of algorithm and runtime budget.
...
This strongly implies that in a general Python developing scenario the usage of the GBOS extension is not advised, as the mean effect size found in the last time step budget of the runs gets a value of \(-0.97\) over all algorithms, contrary to the value of \(0.21\) (in favour of EvoObj) obtained by Yun Lin et al.\ over all the algorithms used in their experimental phase for the maximum budget of 300 seconds. 

\begin{table}[bth]\label{tab:performance}
  \centering
...
The results in Table~\ref{tab:performance}, it is shown once more how the extra computational effort needed while using the GBOS extension influences the obtained coverage results.
This, presented once more distinctly in the results that used the MIO algorithm.
For the time step of 300 seconds, presents an effect size with magnitude of \(10^{12}\), which can be argued by the general amount of computed iterations presented in Figure~\ref{fig:iterations}.
