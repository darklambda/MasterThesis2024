...
a Graph-Based Object Synthesis approach~\cite{DBLP:conf/sigsoft/0001O00D21} (GBOS) and optionally the usage of an external type inference module respectively, in order to diminish the branch coverage gap and try to generate more complex test suites.
...
\begin{figure}
    ...
    \caption{Test suit generated by Pynguin for module example.py\label{lst:3}}
\end{figure}
...
To illustrate this, if line 4 in Listing~\ref{lst:1} were to be replaced to \mint{python}|def checkRules(self, action, state) -> bool:| it would make Pynguin not have any information about the type of the parameters, and generate a test suit similar to the one in Listing\ref{lst:5}.
...
\begin{figure}
  ...
  \caption{Test suit generated by Pynguin for variation of module example.py\label{lst:5}}
\end{figure}
...
The previous setup can be summarized by the following research questions and their experimental implications:
...
The expectation for the result of this research question is to get a Cohen's d effect size greater than \(0.30\).
...
This last research question essentially makes room for two kinds of MUT, those who originally do not have type information in their functions' signatures and should be inferred by using any possible mean to improve the coverage, and those that do have their respective type information and should lose it to check the impact of this loss.
