...
The percentage change (e.g.~\(\frac{\text{GBOSValue} - \text{PynguinValue}}{\text{PynguinValue}}\times 100\)) for the average values of the distributions presented in this plot, are \(-0.006\), \(-0.002\), \(0.005\) and \(-0.013\) respectively, from left to right.
...
Considering a comparison between the Search Time and the Decision Time, MIO obtains a percentage change of \(-31.773\), while the other algorithms have values ranging between \(-99.293\) and \(-98.995\).
With respect to the Generation Time, the comparison using percentage change of MIO gives a value of \(-71.011\), while the other algorithms have values ranging between \(-99.839\) and \(-98.385\).
...
This strongly implies that in a general Python developing scenario the usage of the GBOS extension is not advised nor makes any considerable improvement in coverage results, as the mean effect size found in the last time step budget of the runs gets a value of \(-0.04\) over all algorithms, contrary to the value of \(0.21\) (in favour of EvoObj) obtained by Yun Lin et al.\ over all the algorithms used in their experimental phase for the maximum budget of 300 seconds. 
...
\begin{table}[t]
  ...
  \begin{tabular}{lcccc}\toprule
    ...
    RQ1 After Intersection                          & \(187\)  & \(187\) & \(187\)     & \(187\) \\ 
    \bottomrule
  \end{tabular} 
  \centering 
  \begin{tabular}{lcccc}\toprule
    ...
    \multicolumn{1}{c}{}                            & DynaMOSA & MOSA    & WHOLE SUITE & MIO     \\ \midrule
    Initial Set                                     & \(529\)  & \(523\) & \(551\)     & \(289\) \\
    RQ1 Before Intersection                         & \(389\)  & \(407\) & \(449\)     & \(193\) \\
    RQ1 After Intersection                          & \(187\)  & \(187\) & \(187\)     & \(187\) \\ 
    \bottomrule
  \end{tabular}
...
In addition, the data per time budget step mostly says that during the coverage evolution of Pynguin using MIO, there is persistent difference in performance while using the extension, which can be attributed to the random nature of the Search Based algorithms, or to the inter-iteration time difference in the Search Time variable when using the extension, in the case of the final time step of 600 seconds.
...
In the first and second graphs together, during the whole search time budget it can be remarked how both curves have the same results, following the behaviour already found in Table~\ref{tab:performance}.
This can be backed by the number of runs successfully executed, as all of these modules had a small difference between the both main approaches, ranging from \(2\) to \(7\).
...
\begin{table}[t]
  ...
  \begin{tabular}{lcccc}\toprule
    ...
    RQ2 After Intersection                          & \(32\)   & \(31\)  & \(38\)      & \(8\)   \\ 
    \bottomrule
  \end{tabular} 
  \centering 
  \begin{tabular}{lcccc}\toprule
    ...
    \multicolumn{1}{c}{}                            & DynaMOSA & MOSA    & WHOLE SUITE & MIO     \\ \midrule
    Initial Set                                     & \(529\)  & \(523\) & \(551\)     & \(289\) \\
    RQ2 Before Intersection                         & \(343\)  & \(360\) & \(401\)     & \(145\) \\
    RQ2 After Intersection                          & \(32\)   & \(31\)  & \(38\)      & \(8\)   \\ 
    \bottomrule
  \end{tabular}
