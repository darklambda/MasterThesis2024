...
The current Master's thesis proposes an addition to the original structure of Pynguin~(see~Figure~\ref{fig:pyn}), specifically to the \textit{GenerationAlgorithm} abstract class, by developing a Graph-Based Object Synthesis approach~\cite{DBLP:conf/sigsoft/0001O00D21} (GBOS) for the static analysis of Python at bytecode level in order to generate suitable object inputs, diminish the branch coverage gap, and empirically study the effect of the presence of type information in this same generation heuristic.
...
Pynguin, similarly to other Search-Based test generating tools, represents its test cases as a sequence of implementations of a super class \textit{Statement} that can be later transformed into an Abstract Syntax Tree (AST) and an actual block of Python code.
...
As mentioned before, the main core of Pynguin's test representation is the abstract class \textit{Statement}, and its subsequent wrapper classes \textit{TestCase}, containing a list of \textit{Statements}, and \textit{TestCaseChromosome}, containing a \textit{TestCase}.
In the prior one, the next child classes available are \textit{AssignmentStatement}, a direct assignment of any kind of reference to a variable (e.g.~foo.bar = var\_1); and \textit{VariableCreatingStatement}, representing anything that can be assigned to a variable such as primitive types, collection types, fields, or any type of call (e.g.~int\_1~=~1, list\_1~=~list(), foo~=~Foo(int\_1,~list\_1)).
...
\begin{figure}[h]\label{fig:hierarchy}
  ...
  \vspace*{0.5cm}
  \caption{Hierarchy of the \textit{Statement} class.}
\end{figure}

One class that assures Pynguin's easiness at the time of expanding it, is the abstract class \textit{GenerationAlgorithm}, which works as a layout for the insertion of any new heuristic of algorithm into the tool's current options, for the generation of \textit{TestSuiteChromosome}, another class wrapper for a list of \textit{TestCaseChromosome}.

In the context of evolutionary and genetic algorithms, the \textit{Statement} class has an abstract method called \textit{mutate}, which depending on the instance's final class, mutates either the primitive value, or the references that are encapsulated in it.
...
\begin{enumerate}
  ...
  \item The Test Case Generation step, individually picks functions or methods from the Test Cluster in order to generate instances of the \textit{Statement} class, firstly by generating a certain amount of either primitive, collection or instance statement used as parameter arguments, and then by generating the respective callable, owner of the different branches or lines to cover.
  ...
  \item In the Export step, Pynguin translates the test cases from its \textit{TestCaseChromosome} representation into a Python AST representation and then into a Python test with the correspondent decorators of the library PyTest.
...
The idea of including a dynamic selection of targets starts from the representation of a program into a Control Flow Graph \(\text{CFG} = \langle V_{\text{CFG}}, E_{\text{CFG}} \rangle\) where every node represents an abstraction of statement(s) and every directed edge, a path of execution.
...
The UpdateTarget() procedure, is applied inside Pynguin by implementing a class for, a Control Flow Graph (named \textit{CFG}), as a wrapper of the \textit{ControlFlowGraph} class from the Python \textit{bytecode} module; and a Control Dependency Graph (called \textit{ControlDependenceGraph}).
...
In terms of the Whole Test Suite generation algorithm's implementation in Pynguin, it was done by setting a field in the \textit{WholeSuiteAlgorithm} class called \textit{\_population}, containing a list of \textit{TestSuiteChromosome} objects.
